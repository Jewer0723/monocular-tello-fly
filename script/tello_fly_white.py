...
修改紀錄:
  ...
  [su4.1-yaml] 將飛行速度、控制參數、模型路徑、任務參數集中到 mission_command_tello_white.yaml。
  ...
  [su4.7-dashboard] 將即時影像、MiDaS 深度圖、任務／鍵盤控制資訊整合為單一 Pygame 視窗。
          ...
          - 支援調整視窗大小，版面比例可由 mission_command_tello_white.yaml 的 dashboard 區塊設定。
"""
...
YAML_PATH = os.path.join(BASE_DIR, "mission_command_tello_white.yaml")
...
def _validate_model_files():
    ...
    if missing:
        ...
        for label, path in missing:
            print(f"   - {label}: {path}")
        raise FileNotFoundError("YOLO model file missing. Check mission_command_tello_white.yaml models paths.")
