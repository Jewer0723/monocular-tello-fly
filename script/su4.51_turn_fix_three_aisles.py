...
修改紀錄:
  ...
  [su4.9-turn-fix] 修正走道切換後斜飛問題：
          - 支援 total_aisles，多走道不再寫死 2 條。
          - 每個走道切換步驟結束後短暫懸停，清掉 roll/yaw 慣性。
          - 進入下一走道前執行 POST_AISLE_ALIGN，先對齊 yaw 並停穩，再開始掃描。
"""
...
class AisleInspector:
    def __init__(self, tello, midas, qr_scanner, tracker):
        ...
        # [su4.9] 走道切換穩定化：避免轉完還沒停穩就開始下一條走道掃描，造成斜飛靠近 QR。
        self._total_aisles = int(INSP_CFG.get("total_aisles", 3))
        self._step_settle_sec = float(INSP_CFG.get("aisle_step_settle_sec", 0.35))
        self._post_aisle_align_enabled = INSP_CFG.get("post_aisle_align_enabled", True)
        self._post_aisle_hover_sec = float(INSP_CFG.get("post_aisle_hover_sec", 1.0))
        self._post_aisle_align_speed = int(INSP_CFG.get("post_aisle_align_speed", 18))
        self._post_aisle_align_tol = float(INSP_CFG.get("post_aisle_align_tolerance_deg", 4))
        self._post_aisle_enter_t = 0.0
        self._step_settle_until = 0.0
        self._last_completed_yaw_target = None
    ...
    def snapshot(self) -> dict:
        return {
            ...
            "step_settle_until": self._step_settle_until,
            "last_completed_yaw_target": self._last_completed_yaw_target,
            "align_target_yaw": self._align_target_yaw,
            "post_aisle_enter_t": self._post_aisle_enter_t,
        }
    ...
    def restore(self, snap: dict):
        ...
        self._step_settle_until = snap.get("step_settle_until", 0.0)
        self._last_completed_yaw_target = snap.get("last_completed_yaw_target", None)
        self._align_target_yaw = snap.get("align_target_yaw", None)
        self._post_aisle_enter_t = snap.get("post_aisle_enter_t", 0.0)
    ...
    def _run_turn_to_target(self, yaw_speed: int) -> tuple:
        ...
        if abs(err) <= self._turn_tol:
            self._last_completed_yaw_target = self._turn_target_yaw
            self._turn_target_yaw = None
            return 0, True
    ...
    def _snap_yaw_90(self, yaw: float) -> float:
        """把目前 yaw 對齊到最接近的 90 度方向。
        走道切換後若角度差幾度，先補正再開始下一條走道，避免斜掃。
        """
        return self._norm_ang(round(yaw / 90.0) * 90.0)

    def _begin_post_aisle_align(self):
        self._post_aisle_enter_t = time.time()
        # 優先使用最後一次目標轉向角度；沒有時才對齊最接近的 90 度。
        target = self._last_completed_yaw_target
        if target is None:
            target = self._snap_yaw_90(self._get_yaw())
        self._align_target_yaw = self._norm_ang(float(target))
        self._turn_target_yaw = self._align_target_yaw
        self._state = "POST_AISLE_ALIGN"
        ...
        if self._state == "ROLL_SCAN":
            ...
            if self._qr_count >= self._target_count:
                if self._face == 1:
                    ...
                    if self._aisle_no < self._total_aisles:
                        print(f"✅ 走道{self._aisle_no}兩面掃完，開始繞到走道{self._aisle_no + 1}")
            ...
            # [su4.9] 每個走道切換步驟做完後先短暫懸停，清掉 roll/yaw 慣性。
            if time.time() < self._step_settle_until:
                status = f"AISLE_CHANGE settle step={self._step_idx}"
            ...
            if self._step_idx >= len(self._aisle_steps):
                ...
                self._step_idx = 0
                self._step_settle_until = 0.0
                print(f"🔄 進入走道 {self._aisle_no}，先停穩並校正方向")
                if self._post_aisle_align_enabled:
                    self._begin_post_aisle_align()
                else:
                    self._state = "ROLL_SCAN"
                    self.qr_scanner.start()
            else:
                ...
                if step_done:
                    ...
                    self._step_t = time.time()
                    self._turn_target_yaw = None
                    self._step_settle_until = time.time() + self._step_settle_sec
                    lr = fb = ud = yaw = 0

            status = f"AISLE_CHANGE step={self._step_idx}"

        elif self._state == "POST_AISLE_ALIGN":
            # [su4.9] 進入下一條走道前先對正 yaw + 懸停，避免一開始 roll 掃描時斜衝 QR。
            old_tol = self._turn_tol
            self._turn_tol = self._post_aisle_align_tol
            yaw, aligned = self._run_turn_to_target(self._post_aisle_align_speed)
            self._turn_tol = old_tol

            hover_ok = (time.time() - self._post_aisle_enter_t) >= self._post_aisle_hover_sec
            status = f"POST_AISLE_ALIGN target={self._align_target_yaw} yaw={self._get_yaw():.1f}"
            if aligned and hover_ok:
                print(f"✅ 走道{self._aisle_no}方向已校正，開始掃描")
                self._state = "ROLL_SCAN"
                self._turn_target_yaw = None
                self._align_target_yaw = None
                self.qr_scanner.start()
