"""
main_fly9.6_full_package.py  –  Tello 雙模式任務控制系統（完整版）
=============================================
...
class QRScanner(TargetTracker):
    ...
    def _is_valid_box(self, x1, y1, x2, y2):
        ...
        min_aspect = self.config.get("min_aspect", 0.2)
        max_aspect = self.config.get("max_aspect", 5.0)
        min_area   = self.config.get("min_box_area", 1000)
        max_ratio  = self.config.get("max_area_ratio", 0.80)
    ...
    def process_frame(self, frame):
        # QRScanner 使用 QR/Barcode 模型，所以信心門檻也應該使用 qr_conf，
        # 避免誤用箱體偵測的 box_conf 導致 QR 太難被偵測到。
...
class AisleInspector:
    ...
    def _run_turn_to_target(self, yaw_speed: int) -> tuple:
        ...
        if abs(err) <= self._turn_tol:
            self._turn_target_yaw = None
            return 0, True
        cmd = yaw_speed if err > 0 else -abs(yaw_speed)
        return cmd, False
...
class TelloMissionController:
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("Tello Mission Control v9.6")
    ...
    def run(self):
        ...
        try:
            while self.running:
                ...
                cv2.imshow("Tello Mission Control v9.6", frame)
                if cv2.waitKey(1) == 27:
                    break
