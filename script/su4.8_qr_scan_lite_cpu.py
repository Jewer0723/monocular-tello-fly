...
修改紀錄:
  ...
  [su4.8-lite-qr] 保留 4.7 流程，簡化 QR 掃描程序降低 CPU 負載：
          - QR YOLO 改為低頻偵測並快取 bbox，不再每幀都跑模型。
          - QR 解碼改為輕量 ROI 解碼，移除大量模糊/銳化/自適應閾值前處理。
          - 關閉全畫面 direct decode，避免 CPU 卡死。
"""
...
def decode_qr_from_frame(frame, bbox=None) -> tuple:
    """輕量 QR 解碼版：只解 ROI，避免大量影像前處理造成 CPU 卡死。
    原本版本會對 ROI/全圖做 resize、blur、equalize、sharpen、threshold、adaptiveThreshold、invert 等多種方法；
    這版只保留灰階 + 必要時放大，優先使用 OpenCV QRCodeDetector，再用 pyzbar 補一次。
    """
    ...
    rois = []
    if bbox is not None:
        pad = QR_CFG.get("decode_roi_pad", 35)
        ...
        # 預設不掃全畫面；真的需要 direct decode 時，只掃中央區域，降低 CPU 負擔。
        crop_ratio = float(QR_CFG.get("direct_decode_center_crop_ratio", 0.55))
        h, w = frame.shape[:2]
        cw, ch = int(w * crop_ratio), int(h * crop_ratio)
        x1, y1 = (w - cw) // 2, (h - ch) // 2
        rois.append(frame[y1:y1 + ch, x1:x1 + cw])
    ...
    for roi in rois:
        ...
        h, w = gray.shape[:2]
        min_side = min(h, w)
        if min_side < QR_CFG.get("decode_resize_min_side", 180):
            scale = min(2.0, QR_CFG.get("decode_resize_min_side", 180) / max(min_side, 1))
            candidates.append(cv2.resize(gray, None, fx=scale, fy=scale, interpolation=cv2.INTER_LINEAR))
...
class QRScanner(TargetTracker):
    def __init__(self):
        ...
        # [su4.8] CPU 輕量化：QR YOLO 不再每幀偵測；bbox 短暫快取即可。
        self.yolo_interval_sec = float(QR_CFG.get("yolo_interval_sec", 0.22))
        self.bbox_hold_sec = float(QR_CFG.get("bbox_hold_sec", 0.45))
        self.decode_cooldown_sec = float(QR_CFG.get("decode_cooldown_sec", 0.6))
        self.direct_decode_enabled = QR_CFG.get("direct_decode_enabled", False)
        self.direct_decode_interval_sec = float(QR_CFG.get("direct_decode_interval_sec", 1.0))
        self._last_yolo_t = 0.0
        self._last_direct_decode_t = 0.0
        self._last_det_t = 0.0
        self._cached_det = None  # (cx, cy, area, bbox)
    ...
    def process_frame(self, frame):
        ...
        # [su4.8] 低頻跑 QR YOLO，避免每幀推論造成 CPU/GPU 卡死。
        if now - self._last_yolo_t >= self.yolo_interval_sec:
            det, cx, cy, area, bbox = self.detect_target(frame, conf=qr_conf)
            self._last_yolo_t = now
            if det:
                self._cached_det = (cx, cy, area, bbox)
                self._last_det_t = now
        elif self._cached_det is not None and (now - self._last_det_t <= self.bbox_hold_sec):
            cx, cy, area, bbox = self._cached_det
            det = True

        if det:
            self.target_lost_time = None
            self.has_target = True
            self.last_debug_status = f"QR:LITE area={int(area)} fail={self.consecutive_failures}"
            ...
            # 解碼也降頻；只掃 bbox ROI，不做大量前處理。
            if now - self.last_scan_time > self.config.get("qr_scan_interval_sec", 0.55):
                ok, data = decode_qr_from_frame(frame, bbox)
                if ok and data:
                    qr_decoded, decoded_data = self._record_decode(data, bbox, area)
                    # 掃到後冷卻一下，避免同一張 QR 重複吃 CPU。
                    self.last_scan_time = now + self.decode_cooldown_sec
                else:
                    self.consecutive_failures += 1
                    self.last_debug_status = f"QR:LITE no-decode fail={self.consecutive_failures}"
        ...
        # 預設關閉 direct decode；真的要用時也只掃中央小區域且低頻。
        ...
        self.last_debug_status = "QR:LITE waiting..."
...
class AisleInspector:
    def __init__(self, tello, midas, qr_scanner, tracker):
        ...
        # QR 視覺輔助定高：不使用高度計。
        # [su4.6] 為避免影響解碼，QR 定高只在「QR 夠大且連續穩定出現」後才啟動。
        self._qr_height_enabled = INSP_CFG.get("qr_height_enabled", True)
        self._qr_height_deadzone = INSP_CFG.get("qr_height_deadzone", 70)
        self._qr_height_kp = INSP_CFG.get("qr_height_kp", 0.06)
        self._qr_height_max_ud = INSP_CFG.get("qr_height_max_ud", 5)
        self._qr_height_min_area = INSP_CFG.get("qr_height_min_area", 18000)
        self._qr_height_stable_frames = INSP_CFG.get("qr_height_stable_frames", 3)
        self._qr_height_bbox_shift_px = INSP_CFG.get("qr_height_bbox_shift_px", 45)
        self._qr_height_seen_frames = 0
        self._qr_height_cy_hist = deque(maxlen=5)
        self._qr_height_last_cy = None
    ...
    def _calc_qr_height_ud(self, bbox, area=0) -> int:
        """用 QR 在畫面中的上下位置做弱修正。
        [su4.6] 為了不要降低解碼率，加入三個限制：
        1) QR 框太小時不修正，先讓飛機維持原掃描軌跡。
        2) QR 必須連續穩定出現數幀才修正，避免 bbox 抖動造成上下跳。
        3) 使用 cy 平均值 + 小速度上限，只做很弱的上下微調。
        """
        ...
        if area < self._qr_height_min_area:
            # QR 還太小/太遠時不要上下修，避免還沒解碼就先亂動。
            self._qr_height_seen_frames = 0
            self._qr_height_cy_hist.clear()
            self._qr_height_last_cy = qr_cy
            return 0

        if self._qr_height_last_cy is not None and abs(qr_cy - self._qr_height_last_cy) > self._qr_height_bbox_shift_px:
            # bbox 中心跳太大，當作不穩定，重新累積。
            self._qr_height_seen_frames = 0
            self._qr_height_cy_hist.clear()

        self._qr_height_last_cy = qr_cy
        self._qr_height_seen_frames += 1
        self._qr_height_cy_hist.append(qr_cy)

        if self._qr_height_seen_frames < self._qr_height_stable_frames:
            return 0

        smooth_cy = int(np.mean(self._qr_height_cy_hist))
        err_y = smooth_cy - (FRAME_H // 2)
    ...
    def process(self, frame, depth_norm, center_depth) -> tuple:
        ...
        if self._state == "ROLL_SCAN":
            ...
            if bbox is not None:
                ...
                # [su4.6] 垂直(ud) 只在 QR 夠大且穩定時才弱修正，避免降低 QR 解碼率。
                qr_height_ud = self._calc_qr_height_ud(bbox, area)
            ...
            if self._step_idx >= len(self._aisle_steps):
                ...
                if action == "roll_left":
                    ...
                    # [su4.7] 走道切換穩定用：每次平移/轉向後短暫停住，
                    # 讓 Tello 的慣性與 yaw 漂移收斂，避免下一段斜著飛。
                    lr = fb = ud = yaw = 0
                    step_done = duration > 0 and time.time()-self._step_t >= duration
                else:
                    step_done = True
                ...
                if step_done:
                    # [su4.7] 換步驟瞬間先送停止，避免上一段 roll / yaw 慣性帶進下一段。
                    lr = fb = ud = yaw = 0
                    self._step_idx += 1
                    self._step_t = time.time()
                    self._turn_target_yaw = None
