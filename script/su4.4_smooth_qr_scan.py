...
修改紀錄:
  ...
  [su4.3] 移植 main_fly9.6 高度計定高控制功能：
          ...
          - ROLL_SCAN 階段使用 base_ud（氣壓計 PID）取代 qr_ud（YOLO 垂直偏差）穩定高度。
...
class AisleInspector:
    ...
    def process(self, frame, depth_norm, center_depth) -> tuple:
        ...
        if self._state == "ROLL_SCAN":
            ...
            # 氣壓計高度維持 PID 控制 + 物理突變過濾器 (ud) [su4.3]
            base_ud = 0
            if self._maintain_height:
                current_h = self._get_stable_height()

                offset_sec = self._left_offset_before_hold_sec

                if not self._hold_height_initialized:

                    if time.time() - self._roll_start_time >= offset_sec:
                        self._hold_height_target = current_h
                        self._hold_height_initialized = True

                if self._hold_height_initialized:
                    err_height = self._hold_height_target - current_h
                    if abs(err_height) <= self._height_deadband_cm:
                        raw_ud = 0
                    else:
                        raw_ud = int(err_height * self._height_kp)
                        raw_ud = max(-self._max_ud, min(self._max_ud, raw_ud))
                    # [su4.4] 平滑 ud，避免高度計跳動造成上下抽動
                    self._base_ud_smooth = (
                        self._height_smooth_alpha * raw_ud
                        + (1.0 - self._height_smooth_alpha) * self._base_ud_smooth
                    )
                    base_ud = int(self._base_ud_smooth)
                else:
                    base_ud = 0
                    self._base_ud_smooth = 0.0

            # 預設維持向左飄，並借用 base_fb 穩定距離、base_ud 穩定高度
