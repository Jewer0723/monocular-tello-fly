"""
su4.1.py  –  Tello 雙模式任務控制系統 (去除爬升與右回掃，導入定距控制版)
=============================================
...
class QRScanner(TargetTracker):
    ...
    def _is_valid_box(self, x1, y1, x2, y2):
        w, h = x2-x1, y2-y1
        area, aspect = w * h, w / (h + 1e-5)
        if not (0.15 < aspect < 8.0): return False
        if area / (FRAME_W * FRAME_H) > 0.90: return False
...
class AisleInspector:
    ...
    def process(self, frame, depth_norm, center_depth) -> tuple:
        ...
        if self._state == "ROLL_SCAN":
            ...
            # 預設維持向左飄，並借用 qr_ud 穩定高度，fb 強制使用 MiDaS 計算出的 base_fb
            ...
            if bbox is not None:
                ...
                # 關鍵：前後(fb) 依然強制維持 MiDaS 的 base_fb，丟棄 YOLO 算出的 qr_fb
