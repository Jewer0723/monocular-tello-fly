...
修改紀錄:
  ...
  [su4.4-qr-optimized] QR 掃描穩定化：
          - Mode 2 建議先關閉 hybrid auto-switch，避免掃描途中被箱子偵測切到 Mode 1。
          - QR bbox 鎖定後，左右/旋轉/短距離前後微調改由 QRScanner 接管。
          - QR 太小或連續 no-decode 時會慢速靠近，QR 足夠大時停住解碼。
          - 解碼成功後短暫懸停，降低重複掃描與畫面殘影造成的誤判。
"""
...
try:
    ...
    pyzbar = None
    print(f"⚠️ pyzbar 載入失敗，將只使用 OpenCV QRCodeDetector 解碼：{_pyzbar_err}")
...
class AisleInspector:
    def __init__(self, tello, midas, qr_scanner, tracker):
        ...
        # [su4.4] QR 鎖定後的穩定掃描參數。
        # 重點：看到 QR 時先把 QR 放大、置中、停穩，再解碼。
        self._qr_lock_max_fb = INSP_CFG.get("qr_lock_max_fb_speed", 8)
        self._qr_lock_slow_fb = INSP_CFG.get("qr_lock_slow_forward_speed", 4)
        self._qr_lock_hold_when_ready = INSP_CFG.get("qr_lock_hold_when_ready", True)
        self._scan_pause_after_decode_sec = INSP_CFG.get("scan_pause_after_decode_sec", 0.8)
    ...
    def restore(self, snap: dict):
        ...
        self._pause_until = snap.get("pause_until", 0.0)
    ...
    def process(self, frame, depth_norm, center_depth) -> tuple:
        ...
        if self._state == "ROLL_SCAN":
            ...
            if now < self._pause_until:
                status = (f"ROLL_SCAN_PAUSE A{self._aisle_no}F{self._face} "
                          f"[{self._qr_count}/{self._target_count}]")
            ...
            if bbox is not None:
                ...
                cv2.putText(frame, f"Locked area={int(area)}", (x1, y1 - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)

                # [su4.4] 發現 QR 後，先讓 QRScanner 接管鎖定。
                # 左右/旋轉：把 QR 置中，避免一邊掃一邊滑出畫面。
                # 前後：QR 太小或連續解碼失敗時慢速靠近；面積足夠時停住，讓畫面更穩定。
                qr_height_ud = self._calc_qr_height_ud(bbox)
                lr, ud, yaw = qr_lr, qr_height_ud, qr_yaw

                min_decode_area = QR_CFG.get("min_area_before_decode", 15000)
                target_area = QR_CFG.get("target_area", 30000)
                qr_fb_cmd = max(-self._qr_lock_max_fb, min(self._qr_lock_max_fb, int(qr_fb)))

                if area < min_decode_area:
                    # QR 還太小，優先往前靠近。
                    fb = qr_fb_cmd if qr_fb_cmd != 0 else self._qr_lock_slow_fb
                elif (QR_CFG.get("forward_when_no_decode", True)
                      and self.qr_scanner.consecutive_failures > 0
                      and area < target_area):
                    # 已經看見 QR 但解不出來，通常是太小或太斜，慢速靠近再試。
                    fb = max(1, min(self._qr_lock_max_fb, abs(qr_fb_cmd) or self._qr_lock_slow_fb))
                elif self._qr_lock_hold_when_ready:
                    # QR 已足夠大時停住，不再使用 MiDaS base_fb 抖動距離。
                    fb = 0
                else:
                    fb = base_fb
            ...
            if decoded and data:
                ...
                self.qr_scanner.start()
                self._pause_until = time.time() + self._scan_pause_after_decode_sec
