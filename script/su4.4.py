...
class AisleInspector:
    def __init__(self, tello, midas, qr_scanner, tracker):
        ...
        # 定高控制已移除：ROLL_SCAN 階段不再使用高度計/YOLO 垂直修正，ud 固定為 0。
    ...
    def process(self, frame, depth_norm, center_depth) -> tuple:
        ...
        if self._state == "ROLL_SCAN":
            ...
            # 定高控制已移除：掃描時不上下修正，避免高度計/QR 垂直控制造成忽高忽低。
            base_ud = 0

            # 預設維持向左飄，並借用 base_fb 穩定距離；ud 固定 0。
            ...
            if bbox is not None:
                ...
                # 垂直(ud) 不再交給高度計或 QRScanner，固定 0，避免掃描時上下跑。
