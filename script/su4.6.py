...
# ──────────────────────────────────────────────────────────
#  QR 解碼輔助（CPU 智能優化版：Early-Exit 流程）
# ──────────────────────────────────────────────────────────
def decode_qr_from_frame(frame, bbox=None) -> tuple:
    def _try_decode(img):
        # 引擎 1: pyzbar (速度快，對普通條碼/QR效果好)
        try:
        ...
        # 引擎 2: OpenCV (速度慢，但對變形或破損的 QR 偶爾有奇效)
        try:
    ...
    if bbox is not None:
        ...
        rois.append(frame)  # 兜底全畫面

    # 2. 過關斬將處理流程 (只要成功就立刻 return，不浪費 CPU)
    for roi in rois:
        gray = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY) if len(roi.shape) == 3 else roi
        h, w = gray.shape[:2]

        # 第 1 關：原圖灰階直接解碼 (70% 的 QR 會在這裡過關，幾乎不耗 CPU)
        ok, data = _try_decode(gray)
        if ok: return True, data

        # 第 2 關：放大 (只針對距離太遠、框太小的情況)
        target_for_filters = gray
        if min(h, w) < 220:
            scale = max(2, int(360 / (min(h, w) + 1e-5)))
            target_for_filters = cv2.resize(gray, None, fx=scale, fy=scale, interpolation=cv2.INTER_CUBIC)
            ok, data = _try_decode(target_for_filters)
            if ok: return True, data

        # 第 3 關：自適應二值化 (專治貨架陰影、現場反光不均勻)
        adaptive = cv2.adaptiveThreshold(target_for_filters, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11,
                                         2)
        ok, data = _try_decode(adaptive)
        if ok: return True, data

        # 第 4 關：Otsu 全局二值化 (專治整體對比度極低的昏暗環境)
        _, otsu = cv2.threshold(target_for_filters, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        ok, data = _try_decode(otsu)
        if ok: return True, data

        # 第 5 關：銳化 (專治無人機移動時的輕微動態模糊)
        sharpening = np.array([[-1, -1, -1], [-1, 9, -1], [-1, -1, -1]])
        sharp = cv2.filter2D(target_for_filters, -1, sharpening)
        ok, data = _try_decode(sharp)
        if ok: return True, data
